...
function ChartRenderer({ config, className }: { config: ChartConfig; className?: string }) {
  ...
  const xAxis = (
    <XAxis
      ...
      label={config.x_label ? { value: config.x_label, position: 'insideBottom', offset: -4, fontSize: 12, fill: '#71717a' } : undefined}
    />
  )
  const yAxis = (
    <YAxis
      tick={{ fontSize: 12, fill: '#52525b' }}
      label={config.y_label ? { value: config.y_label, angle: -90, position: 'insideLeft', fontSize: 12, fill: '#71717a' } : undefined}
    />
  )
    ...
    <div className="flex h-full min-h-0 overflow-hidden bg-[#f7f7f8]">
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100">
          <span className="text-sm font-semibold text-zinc-800">Saved Charts</span>
          <Button variant="ghost" size="icon" className="h-6 w-6 text-zinc-400 hover:text-zinc-600" onClick={() => setSidebarOpen(false)}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {savedLoading ? (
            ...
            <p className="text-center text-xs text-zinc-400 py-8">No saved charts yet</p>
          ) : saved.map(viz => (
            <div
              key={viz.id}
              className="group rounded-md p-2.5 hover:bg-zinc-50 cursor-pointer transition-colors border border-transparent hover:border-zinc-200"
              onClick={() => loadSavedViz(viz)}
            >
              <div className="flex items-start justify-between gap-1">
                <div className="min-w-0">
                  <p className="text-xs font-medium text-zinc-800 truncate">{viz.title}</p>
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-0.5 text-zinc-400 hover:text-red-500 transition-opacity flex-shrink-0"
                >
      ...
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex items-center gap-3 border-b border-zinc-200 bg-white px-6 py-4">
          {!sidebarOpen && (
            <Button variant="ghost" size="icon" className="h-7 w-7 text-zinc-400 hover:text-zinc-600 -ml-1" onClick={() => setSidebarOpen(true)}>
              <ChevronRight className="h-4 w-4" />
            </Button>
          )}
          <BarChart2 className="h-5 w-5 text-orange-500 flex-shrink-0" />
          <div>
            <h1 className="text-base font-semibold text-zinc-900">Data Visualizer</h1>
              ...
              <div className="mt-2 grid grid-cols-1 gap-2 text-left max-w-sm w-full">
                {[
                  ...
                  <button
                    ...
                    className="text-left text-xs text-zinc-600 hover:text-zinc-900 bg-white hover:bg-zinc-50 rounded-lg px-3 py-2 transition-colors border border-zinc-200 hover:border-zinc-300"
                  >
                    "{ex}"
                  </button>
                ))}
            ...
            <div className="space-y-4 max-w-4xl mx-auto">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-500 mt-0.5">{config.description}</p>
                </div>
              ...
              {/* Chart */}
              <div ref={chartWrapRef} className="bg-white rounded-xl border border-zinc-200 p-6 shadow-sm">
                <ChartRenderer config={config} />
              </div>

              {/* Data table */}
              <details className="group">
                <summary className="cursor-pointer text-xs text-zinc-500 hover:text-zinc-700 flex items-center gap-1 select-none">
                    ...
                    <tbody className="divide-y divide-zinc-100 bg-white">
                      {config.data.map((row, i) => (
                        <tr key={i} className="hover:bg-zinc-50">
                          {[config.x_key, ...config.y_keys].map(h => (
                            <td key={h} className="px-3 py-2 text-zinc-700">{String(row[h] ?? '')}</td>
                          ))}
                        </tr>
                      ))}
                    </tbody>
